...
**40 agent-routable tools** (EOQ, safety stock, `(s,Q)`/`(R,S)`, simulation,
...
cost-to-serve, S&OP, facility location, DRP, transportation, FEFO, vehicle
routing…) sits under an
...
| `examples/` | Runnable CLIs (`run_agent.py`, `query_knowledge.py`, `run_*_job.py`, `run_price_intel.py`, `run_price_watch.py` (discovery-assisted price watch, end-to-end), parts 1–4) |
