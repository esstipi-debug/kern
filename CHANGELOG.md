...
- **Operations / strategy / sustainability L3 layer** — ingested four authoritative SCM textbooks into the books knowledge graph (`knowledge/scm-books/`): Vollmann *Manufacturing Planning & Control* (S&OP/MPS/MRP/DRP — the planning/manufacturing spine, 149 nodes), Ivanov *Global Supply Chain & Operations* (sourcing/network/risk/digital, 70 nodes), Christopher *Logistics & SCM* (agility/lead-time/network/3PL, 247 nodes), and Grant, Trautrims & Wong *Sustainable Logistics & SCM* (green/reverse logistics/circular economy, 177 nodes). The books graph grew 742 → 1383 nodes / 2539 edges / 100 communities. Vollmann/Ivanov extracted via the Kimi backend; Christopher/Grant via host subagents (no API key) — single canonical graph, concepts merged by label into cross-book bridges.
