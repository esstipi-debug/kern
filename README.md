...
Every one of the **39 registered tools** below is routed the same way — one `register()` call wires a capability into the same `classify → prepare → run → QA → deliver` pipeline, no special-casing. A few, as a sample:
...
## 🧰 All 39 capabilities, one router
...
<details>
<summary><b>📋 The full list, by area (39 tools)</b></summary>
...
brief ─▶ intent.classify ─▶ registry.get(tool) ─▶ prepare ─▶ run ─▶ QA ─▶ deliver ─▶ JobResult
              (rules + optional LLM)              any of the 39 registered tools
```
...
```
scm_agent/            Orchestrator: brief → classify → tool (39 registered) → QA → deliver
