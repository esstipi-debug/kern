"""Tests for the Control Tower webapp surface (Linchpin 3.0 PR-7, plan S5):
...
def isolated_ledgers(tmp_path, monkeypatch):
    """Point the app's Control Tower ledgers at throwaway sqlite files so
    these tests never touch the real (gitignored) data/ directory and never
    see events/records left behind by another test."""
    ...
    return events_path, autonomy_path
...
def test_events_endpoint_returns_real_emitted_events(isolated_ledgers):
    events_path, _ = isolated_ledgers
...
def test_approve_pending_completes_the_underlying_t2_item(isolated_ledgers):
    _, autonomy_path = isolated_ledgers
