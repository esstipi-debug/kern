...
Every one of the **37 registered tools** below is routed the same way — one `register()` call wires a capability into the same `classify → prepare → run → QA → deliver` pipeline, no special-casing. A few, as a sample:
...
<details>
<summary><b>📋 The full list, by area (37 tools)</b></summary>
...
brief ─▶ intent.classify ─▶ registry.get(tool) ─▶ prepare ─▶ run ─▶ QA ─▶ deliver ─▶ JobResult
              (rules + optional LLM)              any of the 37 registered tools
```
...
```
scm_agent/            Orchestrator: brief → classify → tool (37 registered) → QA → deliver
