...
Every one of the **41 registered tools** below is routed the same way — one `register()` call wires a capability into the same `classify → prepare → run → QA → deliver` pipeline, no special-casing. A few, as a sample:
...
## 🧰 All 41 capabilities, one router
...
<details>
<summary><b>📋 The full list, by area (41 tools)</b></summary>
...
| **Pricing & finance** | `pricing` · `repricing` (decision bridge: elasticity + competitor position -> staged proposal) · `price_intelligence` (competitor price position, one-shot) · `price_watch` (recurring discovery-assisted watch) · `financial_kpis` · `cost_to_serve` · `learning_curve` |
...
brief ─▶ intent.classify ─▶ registry.get(tool) ─▶ prepare ─▶ run ─▶ QA ─▶ deliver ─▶ JobResult
              (rules + optional LLM)              any of the 41 registered tools
```
...
```
scm_agent/            Orchestrator: brief → classify → tool (41 registered) → QA → deliver
